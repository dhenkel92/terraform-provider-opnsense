...
func (r *accountResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// Fetch the created account so Computed fields (current_ip, current_mtime) are populated
	createdStruct, err := r.client.Dyndns().GetAccount(ctx, id)
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to read dyndns account after create, got error: %s", err))
		return
	}

	resourceModel, err := convertAccountStructToSchema(createdStruct)
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to parse dyndns account after create, got error: %s", err))
		return
	}

	resourceModel.Id = data.Id
	// Password is not returned by the API, preserve from plan
...
func (r *accountResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Fetch the updated account so Computed fields (current_ip, current_mtime) are populated
	updatedStruct, err := r.client.Dyndns().GetAccount(ctx, data.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to read dyndns account after update, got error: %s", err))
		return
	}

	resourceModel, err := convertAccountStructToSchema(updatedStruct)
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to parse dyndns account after update, got error: %s", err))
		return
	}
